...
def set_environment(select):

    trials_noise_where = 'both' # other options: 'vel' and 'area'
    trials_noise_howmuch = [1, 1, 5, 5, 10, 10]  # % of noise (double because I made 2 trials for each % of noise)
    noise_pct = trials_noise_howmuch[select-1]
    #noise_pct = 0 # uncomment to NOT have noise

    print('I start with {} input data perturbed by {} % of noise ',format(trials_noise_where, noise_pct) )

    layers = [2, 100, 100, 100, 100, 100, 100, 100, 3]

    path = 'trial_noise' + str(select) + '/'

    ####### UNCOMMENT ONE AND ONLY ONE ########
    #restore_backup = './Model/' + path  # uncomment to specify where to load the backup
    ...
    return layers, path, trials_noise_where, noise_pct, restore_backup, backup_file


if __name__ == "__main__":

    select = 2 #from 0 to 5 (see vector trials_noise_howmuch above)
        
    layers, path, noise_loc, noise_pct, restore_backup, backup_file = set_environment(select)
    ...
    if noise_pct != 0:
        if restore_backup != 'NO':
            ...
            try:
               noise_data_velocity = np.load(restore_backup + 'Noise_data_velocity.npy').item()
            except IOError:
                print("File Noise_data_velocity.npy not accessible")

            try:
               noise_data_area = np.load(restore_backup + 'Noise_data_area.npy').item()
            except IOError:
                print("File Noise_data_area.npy not accessible")

            # restore np.load for future normal usage
            np.load = np_load_old

            vel_meas_noise1 = noise_data_velocity['velocity_measurements_vessel1']
            vel_meas_noise2 = noise_data_velocity['velocity_measurements_vessel2']
            vel_meas_noise3 = noise_data_velocity['velocity_measurements_vessel3']

            area_meas_noise1 = noise_data_area['area_measurements_vessel1']
            area_meas_noise2 = noise_data_area['area_measurements_vessel2']
            area_meas_noise3 = noise_data_area['area_measurements_vessel3']

            # Plot a comparative ov noised vs non-noised data for velocity
            ...
            fig_noise = plt.figure(4, figsize=(22, 12), dpi=300, facecolor='w', frameon=False)
            ax_noise1 = fig_noise.add_subplot(131)
            ax_noise1.plot(t, area_measurements_vessel1, 'b-', linewidth=1, markersize=0.5,
                           label='Area without noise 1')
            ax_noise1.plot(t, area_meas_noise1, 'r-', linewidth=1, markersize=0.5, label='Area with noise 1')

            ax_noise2 = fig_noise.add_subplot(132)
            ax_noise2.plot(t, area_measurements_vessel2, 'b-', linewidth=1, markersize=0.5,
                           label='Area without noise 2')
            ax_noise2.plot(t, area_meas_noise2, 'r-', linewidth=1, markersize=0.5, label='Area with noise 2')

            ax_noise3 = fig_noise.add_subplot(133)
            ax_noise3.plot(t, area_measurements_vessel3, 'b-', linewidth=1, markersize=0.5,
                           label='Area without noise 3')
            ax_noise3.plot(t, area_meas_noise3, 'r-', linewidth=1, markersize=0.5, label='Area with noise 3')

            fig_noise.suptitle('Input with white noise error at ' + str(noise_pct) + '%')
            ax_noise1.set_xlabel("t in s")
            ax_noise1.set_ylabel("Area [Vessel 1] in m^2")
            ax_noise2.set_xlabel("t in s")
            ax_noise2.set_ylabel("Area [Vessel 2] in m^2")
            ax_noise3.set_xlabel("t in s")
            ax_noise3.set_ylabel("Area [Vessel 3] in m^2")

            # # Save the plot
            # fig_noise.savefig('./Figures/' + path + "Input with noise (velocity).png")  # Save figure
            ...
            # Update the input vectors
            area_measurements_vessel1 = area_meas_noise1
            area_measurements_vessel2 = area_meas_noise2
            area_measurements_vessel3 = area_meas_noise3
            ...
            del noise_data_velocity, noise_data_area, vel_meas_noise1, vel_meas_noise2, vel_meas_noise3
        ...
        if restore_backup == 'NO':
            ...
            if noise_loc == 'vel' or noise_loc == 'both':
                ...
                noise_data = dict({'velocity_measurements_vessel1': velocity_measurements_vessel1,
                                   'velocity_measurements_vessel2': velocity_measurements_vessel2,
                                   'velocity_measurements_vessel3': velocity_measurements_vessel3})
                np.save('./Model/' + path + 'Noise_data_velocity.npy', noise_data)

                # Free memory
                del vel_meas_noise1, vel_meas_noise2, vel_meas_noise3, temp, L2_norm_velocity, samples1, samples2, samples3, fig_noise

            if noise_loc == 'area' or noise_loc == 'both':

                # Generate the errors
                samples1 = np.random.normal(0, dev_noise, size=area_measurements_vessel1.shape[0])
                temp = np.zeros((area_measurements_vessel1.shape[0], 1))
                for h in range(len(temp)):
                    temp[h] = area_measurements_vessel1[h]**2
                L2_norm_area = np.sqrt(np.mean(temp))
                area_meas_noise1 = np.zeros((area_measurements_vessel1.shape[0], 1))
                for i in range(area_measurements_vessel1.shape[0]):
                    area_meas_noise1[i] = area_measurements_vessel1[i] + L2_norm_area*samples1[i]

                samples2 = np.random.normal(0, dev_noise, size=area_measurements_vessel2.shape[0])
                temp = np.zeros((area_measurements_vessel2.shape[0], 1))
                for h in range(len(temp)):
                    temp[h] = area_measurements_vessel2[h]**2
                L2_norm_area = np.sqrt(np.mean(temp))
                area_meas_noise2 = np.zeros((area_measurements_vessel2.shape[0], 1))
                for i in range(area_measurements_vessel2.shape[0]):
                    area_meas_noise2[i] = area_measurements_vessel2[i] + L2_norm_area*samples2[i]

                samples3 = np.random.normal(0, dev_noise, size=area_measurements_vessel3.shape[0])
                temp = np.zeros((area_measurements_vessel3.shape[0], 1))
                for h in range(len(temp)):
                    temp[h] = area_measurements_vessel3[h]**2
                L2_norm_area = np.sqrt(np.mean(temp))
                area_meas_noise3 = np.zeros((area_measurements_vessel3.shape[0], 1))
                for i in range(area_measurements_vessel3.shape[0]):
                    area_meas_noise3[i] = area_measurements_vessel3[i] + L2_norm_area*samples3[i]

                # Plot a comparative ov noised vs non-noised data
                fig_noise = plt.figure(4, figsize=(22, 12), dpi=300, facecolor='w', frameon=False)
                ax_noise1 = fig_noise.add_subplot(131)
                ax_noise1.plot(t, area_measurements_vessel1, 'b-', linewidth=1, markersize=0.5, label='Area without noise 1')
                ax_noise1.plot(t, area_meas_noise1, 'r-', linewidth=1, markersize=0.5, label='Area with noise 1')

                ax_noise2 = fig_noise.add_subplot(132)
                ax_noise2.plot(t, area_measurements_vessel2, 'b-', linewidth=1, markersize=0.5, label='Area without noise 2')
                ax_noise2.plot(t, area_meas_noise2, 'r-', linewidth=1, markersize=0.5, label='Area with noise 2')

                ax_noise3 = fig_noise.add_subplot(133)
                ax_noise3.plot(t, area_measurements_vessel3, 'b-', linewidth=1, markersize=0.5, label='Area without noise 3')
                ax_noise3.plot(t, area_meas_noise3, 'r-', linewidth=1, markersize=0.5, label='Area with noise 3')
                ...
                # Save the plot
                fig_noise.savefig('./Figures/' + path + "Input with noise (area).png")  # Save figure
                ...
                # Save the perturbed observations
                noise_data = dict({'area_measurements_vessel1': area_measurements_vessel1,
                                   'area_measurements_vessel2': area_measurements_vessel2,
                                   'area_measurements_vessel3': area_measurements_vessel3})
                np.save('./Model/' + path + 'Noise_data_area.npy', noise_data)

                # Free memory
                del area_meas_noise1, area_meas_noise2, area_meas_noise3, temp, L2_norm_area, samples1, samples2, samples3, fig_noise

            # Free memory
            del noise_pct, dev_noise
    ...
    T_residual = lower_bound_t + (upper_bound_t-lower_bound_t)*np.random.random((N_f))[:,None]  #random times for collocations pts.
    ...
    with open("test.txt", "a") as myfile:
        myfile.write("\n TRIAL"+str(select))
        myfile.write("\n L2 error in vessel 1: " + str(error_p1))
        myfile.write("\n L2 error in vessel 2: " + str(error_p2))
        myfile.write("\n L2 error in vessel 3: " + str(error_p3))
        myfile.write("\n")

    del model, fig1, fig2, fig3